...
test("public runtime surfaces R1.4.1 consistently", async () => {
  ...
  assert.match(html, /BUILD<\/span><strong>R1\.4\.1/);
  assert.match(html, /PUBLIC RECEIPT VERIFIER \/ R1\.4\.1/);
  assert.equal(JSON.parse(pkg).version, "1.4.1");
  assert.match(app, /SERVER R1\.4\.1/);
  assert.match(engine, /ENGINE_VERSION = "r1\.4\.1"/);
  assert.match(client, /"x-rewind-engine": "r1\.4\.1"/);
});
