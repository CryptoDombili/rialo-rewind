...
test("public runtime surfaces R1.4 consistently", async () => {
  ...
  assert.match(html, /BUILD<\/span><strong>R1\.4/);
  assert.match(html, /PUBLIC RECEIPT VERIFIER \/ R1\.4/);
  assert.equal(JSON.parse(pkg).version, "1.4.0");
  assert.match(app, /SERVER R1\.4/);
  assert.match(engine, /ENGINE_VERSION = "r1\.4"/);
  assert.match(client, /"x-rewind-engine": "r1\.4"/);
});
